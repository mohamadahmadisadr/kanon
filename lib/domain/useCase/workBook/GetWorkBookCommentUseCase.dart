...
class GetWorkBookCommentUseCase extends BaseUseCase {
  ...
  void getByTestDate({
    ...
    try {
      ...
      Uri uri = UriCreator.createUri(
          path: WorkBookApiRoutes.getWorkBookComment, body: map);
      Logger.d(map);
      var response = await apiServiceImpl.post(uri);
      Logger.d(response.statusCode);
      if (response.isSuccessful) {
        var examDetail = getWorkBookCommentResponseFromJson(response.body);
        if (examDetail.status == 1 && examDetail.data?.state == 1 ||
            examDetail.status == 1 && examDetail.data?.state == 4) {
