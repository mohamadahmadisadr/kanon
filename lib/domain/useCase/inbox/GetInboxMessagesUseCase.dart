...
import '../../../data/serializer/inbox/GetInboxMessagesResponse.dart';

class GetInboxMessagesUseCase extends BaseUseCase {
  ...
  void invoke({MyFlow<AppState>? flow, Object? data}) async {
    try {
      ...
      Uri uri = UriCreator.createUri(
          path: InboxApiRoutes.getInboxMessages, body: userId.toJson());
      Logger.d(userId.toJson());
      var response = await apiServiceImpl.post(uri);
      Logger.d(response.statusCode);
      if (response.isSuccessful) {
        var examDetail = getInboxMessagesResponseFromJson(response.body);
