...
class GetEBookListUseCase extends BaseUseCase {
  ...
  void invoke({MyFlow<AppState>? flow, Object? data}) async {
    try {
      ...
      Uri uri = UriCreator.createUri(
          path: BooksApiRoutes.getEBookList, body: userId.toJson());
      Logger.d(userId.toJson());
      var response = await apiServiceImpl.post(uri);
      Logger.d(response.statusCode);
      if (response.isSuccessful) {
        var examDetail = getEBookListResponseFromJson(response.body);
